...
public class AtdGrpc extends Activity {
    ...
    public void getFeature(View view) {
        ...
        Bitmap bitmap = BitmapFactory.decodeResource(getResources(), R.drawable.yene);

        new GrpcTask(new GetClassRunnable(), bitmap, channel, this).execute();
    }
    ...
    private static class GrpcTask extends AsyncTask<Void, Void, String> {
        ...
        private final WeakReference<AtdGrpc> activityReference;

        GrpcTask(GrpcRunnable grpcRunnable,Bitmap image, ManagedChannel channel, AtdGrpc activity) {
            this.grpcRunnable = grpcRunnable;
            this.channel = channel;
            this.image = image;
            this.activityReference = new WeakReference<AtdGrpc>(activity);
        }
        ...
        protected void onPostExecute(String result) {
            AtdGrpc activity = activityReference.get();
            if (activity == null) {
    ...
    private interface GrpcRunnable {
        ...
        String run(Bitmap image, ImageClassBlockingStub blockingStub, ImageClassStub asyncStub) throws Exception;
    }

    private static class GetClassRunnable implements GrpcRunnable {
        @Override
        public String run(Bitmap image ,ImageClassBlockingStub blockingStub, ImageClassStub asyncStub)
                throws Exception {
            String ret  ;
            ret =getFeature(ByteString.copyFrom(getBytesFromBitmap(image)), blockingStub);
            if (ret==null){
        ...
        private String getFeature(ByteString image , ImageClassBlockingStub blockingStub)
            ...
            GrpcRequest request = GrpcRequest.newBuilder().setImage(image).build();
